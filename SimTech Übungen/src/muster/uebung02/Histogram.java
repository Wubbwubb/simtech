 ...
 *
 * Histogram class Basic abstract class for all histograms Defining interface and functions of histograms
 * 
...
public abstract class Histogram {
	 ...
	 *            number of bounds
	 */
	public Histogram(String oVariable, String fname, double lB, double uB, int num) {
	...
	/**
	 * Function sets the number of intervals to the value of the given argument i and resets the vector.
	 * 
	 * @param i
	...
	/**
	 * Function tries to write the counted values to three files. Filename is the name of the observed variable +
	 * extension. The first file with extension ".hist" represents the usual histogram. The second file with extension
	 * ".epdf" represents the probability density function. The third file with extension ".dist" represents the
	 * distribution.
	 * 
	 * @throws Exception
	...
	public void report_lower_upper(String theFilename) {
		...
		try {
			...
			for (int i = 0; i < this.numIntervals; i++) {
				ohist.write(("" + String.valueOf(this.lowerBound + i * this.delta) + " , "
						+ String.valueOf(this.lowerBound + (i + 1) * this.delta) + " , " + this.bins.get(i) + "\n")
						.getBytes());
			}
			ohist.close();
		} catch (Exception e) {
	...
	public void report(String theFilename) {
		...
		try {
			FileOutputStream ohist = new FileOutputStream(fhist);
			for (int i = 0; i < this.numIntervals; i++) {
				ohist.write(("" + String.valueOf(this.lowerBound + (i + 0.5) * this.delta) + " , " + this.bins.get(i)
						/ sum + "\n").getBytes());
			}
			ohist.close();
		} catch (Exception e) {
