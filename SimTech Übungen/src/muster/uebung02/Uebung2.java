...
import muster.uebung01.RandVar;
import muster.uebung01.SimGIGIS;

/**
 * Exercise 2 Simulationstechnink SS2015
 * 
 * Uebung2 class Mainclass performing the simulation.
 * 
...
public class Uebung2 {
	...
	private static TimeIndependentHistogram hist_mean_queue_length = new TimeIndependentHistogram(
			"mean queue length per run", "ql per run.txt", -0.5, 7.5, 8);
	...
	private static TimeIndependentHistogram hist_mean_waiting_time = new TimeIndependentHistogram(
			"mean waiting per run", "wt per run.txt", 0, 5000, 50);
	...
	public static void main(String[] args) {
		...
		for (int S = 5; S <= max_queue; S++) {
			for (int run = 0; run < number_of_runs; run++) {
				...
				counter_mean_queue_length.count(CounterCollection.counter_queuelength.getMean());
				hist_mean_queue_length.count(CounterCollection.counter_queuelength.getMean());
			...
			hist_mean_waiting_time.report("hist_mean_waiting_time_" + simtime + "_" + S + ".txt");
			if (doPrintBufferContent) {
				System.out.println("Buffer size: " + S + " , simulation time: " + simtime + ", Mean buffer content: "
						+ counter_mean_queue_length.getMean() + " Variance: " + counter_mean_queue_length.getVariance()
						+ " Coefficient of Variation: " + counter_mean_queue_length.getCvar());
			}
			if (doPrintWaitingTime) {
				System.out.println("Buffer size: " + S + " , simulation time: " + simtime + ", Mean waiting time: "
						+ counter_mean_waiting_time.getMean() + " Variance: " + counter_mean_waiting_time.getVariance()
						+ " Coefficient of Variation: " + counter_mean_waiting_time.getCvar()
